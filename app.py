...
def get_gemini_model(_api_key: str):
    """
    Automatically pick the strongest available Gemini model.
    Priority order:
    1. Gemini 2.5 Pro Preview (latest)
    2. Gemini 1.5 Pro (latest)
    3. Experimental builds (exp)
    4. Gemini Pro / Pro Vision (fallback)
    """
    try:
        genai.configure(api_key=_api_key)
        available = [m.name for m in genai.list_models() if "generateContent" in m.supported_generation_methods]

        # 🧠 Ordered from most capable to fallback
        priorities = [
            "models/gemini-2.5-pro-preview-03-25",  # newest high-quota model
        ...
        pick = next((m for p in priorities for m in available if p in m), None)

        if not pick:
            st.error("❌ No suitable Gemini model found for your API key.")
            return None, None

        st.success(f"✅ Using Gemini model: **{pick}**")
